...
The immutable development baseline is `benchmarks/corpus_baseline_v1_2026-08-11` and records 44 papers that reached inventory processing: 37 `UNRESOLVED_INVENTORY` and 7 `COMPLETE`.
...
The first analysis of the frozen state found the largest recurring signals to be:
...
These categories overlap. They are diagnostic signals, not ground-truth causal labels.
...
The old broad hourly literature acquisition loop is paused. New literature acquisition resumes only after the extractor has materially improved and the v1 candidate has been validated.
