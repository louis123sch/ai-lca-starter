...
LOCATION_ALIASES = {
    ...
    "norway": "NO", "no": "NO", "europe": "RER", "rer": "RER",
    "global": "GLO", "glo": "GLO", "rest of world": "RoW", "row": "RoW",
}
...
def _query_variants(query: str, activity_type_hint: str | None = None, technology_hint: str | None = None) -> list[str]:
...
def _score_candidate(candidate: dict, *, query: str, preferred_locations: list[str], unit: str | None, activity_type_hint: str | None, technology_hint: str | None, retrieval_rank: int) -> tuple[float, list[str]]:
    ...
    if q and q == product:
        score += 28; reasons.append("reference product exactly matches search concept")
    elif q and q in product:
        score += 20; reasons.append("reference product contains search concept")
    if q and q in name:
        score += 12; reasons.append("activity name contains search concept")
    ...
    if candidate_location in preferred_locations:
        ...
        score += max(4, 18 - 3 * location_rank); reasons.append(f"preferred geography {candidate_location}")
    if unit and (candidate.get("unit") or "").strip().lower() == unit.strip().lower():
        score += 10; reasons.append("unit matches extracted exchange")
    if activity_type_hint and activity_type_hint != "unknown" and _activity_type(candidate.get("name") or "") == activity_type_hint:
        score += 14; reasons.append(f"activity type matches {activity_type_hint} hint")
    if technology_hint:
        overlap = _tokens(technology_hint) & _tokens(f"{candidate.get('name', '')} {candidate.get('reference_product', '')}")
        if overlap:
            score += min(10, 2.5 * len(overlap)); reasons.append("supplier/technology wording overlaps")
    return score, reasons


def search_candidates(*, project_name: str, database_name: str, query: str, locations: Iterable[str] | None = None, limit: int = 12, unit: str | None = None, activity_type_hint: str | None = None, technology_hint: str | None = None) -> list[dict]:
    ...
    if not query:
        return []
    preferred_locations = list(dict.fromkeys(_normalise_location(x) for x in (locations or []) if x and x.strip()))
    ...
    for variant in variants:
        for act in db.search(variant, limit=pool_limit):
            ...
            if key not in seen:
                seen.add(key); collected.append((act, retrieval_rank)); retrieval_rank += 1
    ...
    for act, rank in collected:
        ...
        score, reasons = _score_candidate(candidate, query=query, preferred_locations=preferred_locations, unit=unit, activity_type_hint=activity_type_hint, technology_hint=technology_hint, retrieval_rank=rank)
