...
6a. Shared foreground activities: when the source explicitly describes certain activities as shared or retained as named foreground exchanges across multiple assessed configurations (for example, a shared storage or transport activity that the study keeps as a foreground exchange within each configuration), do NOT create a separate top-level foreground process for that activity. Instead represent it as a named foreground exchange attached to each configuration/process the source associates it with. Only create a separate foreground process if the paper models that activity independently with its own reference product or provides explicit quantified reference flows connecting it to other foreground processes.
